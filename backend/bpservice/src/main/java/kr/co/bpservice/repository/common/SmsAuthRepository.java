...
public interface SmsAuthRepository extends JpaRepository<SmsAuth, Integer> {
    ...
    @Query(value = "SELECT * FROM MWS_SMS_AUTH WHERE (TIMESTAMPDIFF(MINUTE, NOW(), EXP_DT) BETWEEN -5 AND 0) and AUTH_NUM LIKE :auth_num and PHONE_NUM LIKE :phoneNum and STATUS = 0;", nativeQuery = true)
